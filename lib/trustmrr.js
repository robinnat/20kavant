      ...
      await Promise.all(
        STARTUPS.map(async (s, i) => {
          try {
            ...
            const mrrCents = pickMrr(d);
            const rev30Cents = pickRevenue30(d);
            const cents = mrrCents + rev30Cents;
            console.log(
              `[TrustMRR] ${s.slug}: mrr=${mrrCents}c + 30j=${rev30Cents}c → $${Math.round(cents / 100)}`
            );
            return {
              name: d?.name ?? s.slug,
              src: s.src ?? "Stripe",
